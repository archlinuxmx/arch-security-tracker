...
class GroupForm(BaseForm):
    ...
    affected = StringField(u'Affected', validators=[DataRequired(), Length(max=32), Regexp(pkgver_regex)])
    fixed = StringField(u'Fixed', validators=[Optional(), Length(max=32), Regexp(pkgver_regex)])
    ...
    notes = TextAreaField(u'Public notes', validators=[Optional(), Length(max=CVEGroup.NOTES_LENGTH)])
